...
#### Seasonal and Yearly-Seasonal %change summaRY CSV file creation #### -----------------------------------------------------------------------------------------------------------------
...
setwd('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/New_Runs') 
options(warn = 1) # prints warnings as they occur

files <- list.files() #list of all files in folder
files_1 <- files[grepl('1ft', files)] # pulls out 1 foot files
files_tenth <- files[grepl('0.2ft', files)] # pulls out 0.2 foot files

# Columns names for each file are are funky so reformat for R
New_colnames <- c("Start_date","End_date","Duration", "Start_elevation", "End_elevation", "Percent_Diff")
...
# Definitions to change depending on which set of files/folder you want to use
 ...
 filesMain <- files_1 # uncomment one filesMain
 folder <- '1ft' # uncomment one folder
 
# filesMain <- files_28 # uncomment one filesMain
# folder <- '28 days' # uncomment one folder
#******************************************************************************************************
...
for (i in 1:length(filesMain)){ # cycle through each file in set
  ...
  setwd(paste('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/Results/', folder, sep = '')) #adjust directory for this iteration
  ...
  for (j in 1:length(fileTrip)){
    data <- read.csv(paste('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/New_Runs/', fileTrip, sep = '')) # read in the csv
...
# Folder where all the needed files are
 ...
 folder2 <- '1ft' # uncomment one folder2
# folder2 <- '28 days' # uncomment one folder2

#******************************************************************************************************

setwd(paste('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/Results/', folder2, sep = ''))
options(warn = 1) # prints warnings as they occur

files2 <- list.files() #list of all files in folder
files_sznl <- files2[grepl('_Seasonal', files2)] # pulls out 7_Days files
files_yrsznl <- files2[grepl('_YearlySeasonal', files2)] # pulls out 28_Days files
...
for (i in 1:length(filesMain2)){ # cycle through each file in set
  ...
  setwd(paste('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/Results/', folder2, sep = '')) #adjust directory for this iteration
  ...
  # inner loop that manipulates each file to get a seasonal % change summary
  for (j in 1:length(fileTrip)){
    data <- read.csv(paste('/Users/rdel1cmc/Desktop/rdel1cmc/Desktop/Carra_ACE-IT_computer/wetlands_and_coastal/todd/bureau_of_reclamation/FY21_Info/Yale_Project/YaleProject/src/Results/',folder2,'/',fileTrip, sep = ''))[,-1] # read in the csv
    data$Season <- factor(data$Season, levels = c('Spring','Summer','Fall','Winter'))
    
    if(grepl('_Seasonal', fileTrip)) {
    ...
    if(grepl('_YearlySeasonal', fileTrip)) {
